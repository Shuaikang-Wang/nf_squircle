...
goal_pose_list = goal_pose_list = [[4.0, 7.5, np.pi / 2]]

max_step = 10000

file_path = 'DATA/pickle_data/'
current_date_execution = "execution_2024_04_18"
...
for frame in range(current_frame, max_step):
    ...
    # test save data with 10 step
    # if current_frame == 1099:
    ...
    #     break
        ...
        # main_execute.robot.current_goal([3.7499927218057616, 3.7440656780248793, -0.7966273824092553])
        # main_execute.robot.pose[2] = main_execute.robot.pose[2] + np.pi

    # save data every 20 step
    if current_frame % 200 == 0:
        with open(folder_path_execution + '/execution.pickle', 'wb') as f:
            execution_data = execution_data[-10:]
            pickle.dump(execution_data, f)
        time.sleep(5)

    if np.linalg.norm(np.asarray(goal_pose_list[-1][:2]) - main_execute.robot.pose[:2]) <= 0.05:
