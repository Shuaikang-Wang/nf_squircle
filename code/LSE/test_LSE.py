...
import sensors
import features
from line_process import Line_process
...
#-----------------------------------  LINE_PROCESS  ------------------------------------------------------------------------------------------
            if not gcs.all_line_segment:
                gcs.all_line_segment = FeatureMAP.FEATURES
            else:
                for i in range(0,len(FeatureMAP.FEATURES)):
                    flag = 0#表示暂时无法合并
                    for j in range(0,len(gcs.all_line_segment)):
                        New_Line = gcs.merge_2line(gcs.all_line_segment[j],FeatureMAP.FEATURES[i])
                        if New_Line:
                            print("combine!")
                            gcs.all_line_segment[j] = New_Line
                            flag = 1
                    if flag == 0: #表示无法合并则需要添加至gcs.all_line_segment末尾
                        gcs.all_line_segment.append(FeatureMAP.FEATURES[i])
            for feature in gcs.all_line_segment:
...
#--------------------------------------------------------------------------------------------------------------------
        ...
        if detect_landmarks:  # draw all stored landmarks#画直线的部分
            for landmark in FeatureMAP.LANDMARKS:
                ...
                pass
        print("gcs",gcs.all_line_segment)
        environment.map.blit(environment.infomap, (0, 0))
        pygame.display.update()
