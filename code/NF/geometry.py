...
class Rectangular(object):
    ...
    #     return (1 / (height / 2)**2) * (((height / width) * (q[0] - center[0]))**2
    #             + (q[1] - center[1])**2 + (((height / width) * (q[0] - center[0]))**4
    #             + (q[1] - center[1])**4 + ((2 - 4 * s**2) * ((height / width)
    #             * (q[0] - center[0]))**2 * (q[1] - center[1])**2))**0.5) / 2 - 1

    # new obstacle function

    # def unit_potential(self, q: np.ndarray, s = 0.999) -> float:
    #     return (np.linalg.norm(q)**2 + (np.linalg.norm(q)**4 - 4 * s**2 * (q[0] * q[1])**2)**0.5) / 2 - 1
    #
    # def similar_trans(self, q: np.ndarray) -> np.ndarray:
    #     scaleX, scaleY = self.width / 2, self.height / 2
    #     return np.array([(q[0] - self.center[0]) / scaleX, (q[1] - self.center[1]) / scaleY])
    #
    # def potential(self, q: np.ndarray) -> float:
    #     return self.unit_potential(self.similar_trans(q))
    ...
    # def check_point_inside(self, q: np.ndarray, threshold=0.1, s=0.99):
...
class Workspace(Rectangular):
    ...
    # def potential(self, q: np.ndarray, s = 0.999) -> float:
    #     center, height, width  = self.center, self.height, self.width
    #     return 1 - (1 / (height / 2)**2) * (((height / width) * (q[0] - center[0]))**2
    #                                         + (q[1] - center[1])**2 +
    #                                         (((height / width) * (q[0] - center[0]))**4
    #                                         + (q[1] - center[1])**4 +
    #                                         ((2 - 4 * s**2) * ((height / width)
    #                                         * (q[0] - center[0]))**2
    #                                         * (q[1] - center[1])**2))**0.5) / 2

    # def unit_potential(self, q: np.ndarray, s = 0.999) -> float:
    #     return 1 - (np.linalg.norm(q)**2 + (np.linalg.norm(q)**4 - 4 * s**2 * (q[0] * q[1])**2)**0.5) / 2
    #
    # def similar_trans(self, q: np.ndarray) -> np.ndarray:
    #     scaleX, scaleY = self.width / 2, self.height / 2
    #     return np.array([(q[0] - self.center[0]) / scaleX, (q[1] - self.center[1]) / scaleY])
    ...
    # def check_point_inside(self, q: np.ndarray, threshold=0.09, s=0.999):
    #     x, y, x_0, y_0, a, b = q[0], q[1], self.center[0], self.center[
    #         1], self.width / 2 - threshold, self.height / 2 - threshold
